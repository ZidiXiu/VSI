...
from utils.preprocessing import one_hot_encoder, formatted_data_original, one_hot_indices, missing_proportion, \
...
def generate_data():
    ...
    num_examples = int(0.80 * data_frame.shape[0])    
    train_idx = idx[0: num_examples]
    split = int((data_frame.shape[0] - num_examples) / 2)

    test_idx = idx[num_examples: num_examples + split]
    valid_idx = idx[num_examples + split: data_frame.shape[0]]

    cate_idx = np.where(np.isin(data_frame.columns.values, np.array(one_hot_encode_list)))[0]
    cts_idx = np.setdiff1d(np.arange(data_frame.shape[1]), cate_idx)
    ...
    data_frame.fillna(impute_dict, inplace=True)
...
def outcome_formatted_data(x, t, e, idx, name, data_type, path, outcomes):
    survival_data = formatted_data_original(x=x, t=t, e=e, idx=idx)
